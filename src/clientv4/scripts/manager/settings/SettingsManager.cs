...
public class SettingsManager : ISettingsManager, IDisposable, IReset, IArchive {
    ...
    private SettingsManager() {
        var languageDict = TranslationServer.GetLoadedLocales().ToDictionary<string, GetString, string>(
            locale => () => {
                if (locale == TranslationServer.GetLocale()) {
                    // why so write as that? because it can be generated by gettext
                    return I18N.Tr("core.gui", "locale.name");
                }
                return TranslationServer.GetTranslationObject(locale).GetMessage("locale.name", "core.gui");
            },
            locale => locale
        );
        ...
        AddCoreSetting(new SettingDefine {
            ...
            Config = new InputSetting {
                Placeholder = () => I18N.Tr("core.gui", "settings.game.uuid.placeholder"),
            },
            Value = "",
            DefaultValue = () => "",
            Name = () => I18N.Tr("core.gui", "settings.game.uuid"),
            Description = () => I18N.Tr("core.gui", "settings.game.uuid.description"),
            OnChange = value => {
                ServerStartupConfig.instance.uuid = value;
                SetSettingValue(CoreSetting, "game.uuid", value);
                SaveCoreSettings();
            },
            ExtraButtons = [
                new ExtraButton {
    ...
    public void SetSettingValue(string module, string key, string value) {
        if (!_settings.TryGetValue(module, out var settingList)) return;
        var index = settingList.FindIndex(item => item.Key == key);
        if (index == -1) return;
        var setting = _settings[module][index];
        setting.Value = value;
        _settings[module][index] = setting;
    }
    ...
    public void ReloadCoreSettings() {
        ...
        if (settingsFromFile != null) {
            foreach (var kvp in settingsFromFile) {
                ...
                _settings[CoreSetting][target].OnChange(item.Value);
            }
        } else {
        ...
        if (getDataFunc(SettingsFile) is { } data) {
            ...
            foreach (var level1 in settingsFromFile) {
                ...
                foreach (var level2 in level1.Value) {
                    ...
                    _settings[level1.Key][target].OnChange(item.Value);
                }
            }
        } else {
            Reset();
